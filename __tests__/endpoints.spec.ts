...
const sampleNavigationType = "main-navigation";
...
describe("endpoints", () => {
  ...
  describe("getNavigationEndpoint", () => {
    it("should return navigation endpoint", async () => {
      const result = getNavigationEndpoint(sampleNavigationType);
      expect(result).toEqual(
        `/store-api/v1/navigation/${sampleNavigationType}/${sampleNavigationType}`
      );
    });
  });
